...
- [x] Topic 2 — What kind of IPG servicing is required — _drafted 2026-07-16; open points settled 2026-07-19; verification APIs added and named 2026-07-19; dealer-commission economics trimmed to §3's altitude 2026-07-19_
- [x] Topic 3 — The problem with the offer provided by the Bank — _drafted 2026-07-16; revised 2026-07-19 (Easebuzz-cost argument dropped, dealer margin figures removed, UPI reason answered, capability questions turned into asks); extended 2026-07-19 (pilot at one pump as the counter-offer, HPCL eDFS rollout ask, dealer deposits, aggregator-structure question); tightened 2026-07-19 (interest split removed, eDFS list ask dropped, aggregator question reduced to a question). Volume projections for 10–15 dealers awaited — data being supplied in the next edit, and they go in last._
...
| **Aadhaar**         | Identity of the individual signing up on behalf of the firm or the outlet.                                         |
| **Vehicle RC**      | That a vehicle presented against an order is registered to the customer firm.                                      |
| **Driving licence** | That the person taking delivery of fuel against an order holds a valid licence.                                    |

We ask the Bank to avail these from the aggregator and to negotiate their commercials on the same footing as the gateway charges — and to tell us which of them Easebuzz in fact offers.
...
What cannot be carried is a charge that scales with value. The tickets on this platform are large, running from tens of thousands of rupees to ₹5 lakh, and a percentage charge grows with every one of them while nothing on the dealer's side moves with it. **A percentage of the transfer is therefore not a charge this business can carry**, and the burden grows, not shrinks, as ticket sizes rise.
...
**We therefore ask the Bank to help us reach them** — by putting the program to those dealers itself, or by approaching them together with us, whichever the Bank prefers. We are not asking the Bank for any customer information: **what we are asking for is the Bank's channel, not its data.**
...
5. **The verification services listed in Topic 2 — bank account, PAN, GSTIN, Aadhaar, vehicle RC and driving licence — can be enabled on this account and priced alongside the gateway**, and which of them the aggregator in fact offers.
...
### A question we would like to put to the Bank

One question, asked to keep a door open rather than to change anything now. **Should VSYST collect the payments and settle out to the dealers, instead of the aggregator settling to each dealer directly — and what would such a structure involve?**

We are not proposing it, and nothing else in this document depends on the answer. We ask because the Bank sees far more of these arrangements than we do, and we would rather know early whether the shape is even open to us than build on the assumption that it is.
...
_(to be added — the estimate data is being supplied in the next edit of this file; collection template: `dealer-volume-data.xlsx`)_
...
- **Hold the virtual-account fallback in reserve — confirmed.** _(2026-07-19)_ If the confirmations we are asking for come back negative on the ₹5 lakh ticket or the maker–checker account, the product that fits may be **bank-transfer collection over virtual accounts** — the customer pushes a real NEFT/RTGS from their own channel to a per-invoice virtual account, and the aggregator auto-matches it and confirms back to DZZLO. That carries any ticket size, works from a maker–checker account, and is typically priced flat per collection. It closes the ledger and removes reconciliation, but it keeps beneficiary management — a partial answer to Topic 1, not a full one. **Do not raise it until the confirmations come back**, or the Bank may take it as the ask and leave net banking where it is.
- **Topic 2 trimmed to the same altitude as §3 — done.** _(2026-07-19)_ §2 no longer describes the dealer's commission as a thin slice an MDR would consume. Both sections now carry only the structural point: pump prices are set by the OMCs and the dealer's commission is fixed per litre, so there is no price he can raise — and at these ticket sizes a charge that scales with value cannot be carried, whatever the rate. **The dealer's own economics stay out of the Bank's view**; do not reintroduce them in the meeting either.
- **The interest split is out. The deposit stays — and still needs a defined mechanism.** _(2026-07-19)_ Sharing the interest on dealer deposits between the Bank and VSYST has been removed from §3 and **is not to be raised in the meeting**: if the deposit is the dealer's own money, the interest is ordinarily the dealer's too, and a share going to VSYST needs a structure the dealer has agreed to. That conversation is not ready. The deposit itself remains part of what we put on the table, and a bank will still ask whose money it is and what it secures. **Settle which of these it is** — a security deposit VSYST collects and places with UBI, a minimum balance the dealer maintains in his own account, or a term deposit lien-marked to the program — **and the amount per dealer**, before the offer is made.
- **The eDFS dealer list is no longer being asked for.** _(2026-07-19)_ A bank generally cannot hand a third party a list of its customers, and asking puts the Bank in the position of refusing us on the first request. §3 now asks only for the Bank's channel — that it put the program to those dealers itself, or approach them with us. **Do not ask for the list in the meeting either**, even if the conversation seems to invite it; the channel is what we actually need and the joint approach gets it. Separately, **confirm the eDFS description before submission** — the document asserts only that UBI extends the facility to HPCL dealers, which is what we were told and nothing beyond it.
- **The verification APIs are named — done.** _(2026-07-19)_ §2 now carries the specific list, with bank account verification first: bank account, PAN, GSTIN, Aadhaar, vehicle RC and driving licence. The Bank has to carry a specific list to the aggregator before it can negotiate anything, and a category is not something it can price. Confirmation #5 in §3 names the same six. **Two things to watch:** the stated purpose of each is ours, not the aggregator's wording — read them once more before submission — and the ask includes which of the six Easebuzz in fact offers, so a "no" on one is an answer rather than a failure.
- **The aggregator question is now just a question.** _(2026-07-19)_ The section no longer sets out what such a structure would give us — it asks whether the shape is open to us and leaves it there, so the door stays open without our appearing to want through it. **Ask it that way in the room too.** The answer sits in the RBI's PA framework and the Bank will hear it that way, so go in knowing the present model — aggregator settles to each dealer directly, VSYST touches no customer money — is the clean default, and say so, or it will sound like a description of something we already do.
- **The pilot offer and the projection must arrive together.** _(2026-07-19)_ §3 now offers both: here is our estimate, and here is one outlet on which the Bank can verify it. The pairing is strong only if the estimate is ready — **if the volume data is not gathered in time, the pilot becomes the whole ask, and the Bank prices a waiver against nothing.** The estimate data is being supplied in the next edit of this file. Settle which outlet runs the pilot, and propose how long it runs before net banking opens wider rather than leaving that open for the Bank to set. Note also that the pilot's outputs are worded so that "cost" means what the waiver costs on a real month's volume — the Bank's own number, useful to it — and not a probe into the aggregator's pricing. Keep it that way, per the point above.
