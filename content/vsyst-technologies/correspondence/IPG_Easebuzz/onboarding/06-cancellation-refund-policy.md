...
Payments made through the DZZLO platform to a petrol pump dealer using the payment gateway, and subscription fees paid to VSYST when they are introduced.

## 2. Nature of the transactions

Payments on DZZLO settle invoices raised by a dealer for fuel dispensed at the dealer's outlet against a verified order. Delivery takes place at the outlet at the time of dispensing; nothing is shipped. VSYST is a technology platform: it is not the seller, it does not receive or hold any payment, and the payment gateway settles each payment directly to the dealer's own bank account.
...
The dealer is the seller and the recipient of the payment, so a refund is decided and raised by the dealer.
...
If the same invoice is paid twice, the second payment is refunded by the dealer as in section 4 or, at the customer's request, retained by the dealer as an on-account advance against future invoices.

## 7. Chargebacks

A dispute raised by a customer with its bank is handled through the payment gateway's chargeback process. The dealer responds with the order, delivery and invoice records from the common ledger; VSYST supplies these records.
