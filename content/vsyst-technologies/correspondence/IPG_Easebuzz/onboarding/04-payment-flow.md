...
| 5   | Customer | Invoices → select invoices → New Payment | Records the payment: invoices paid, amount, payment mode (NEFT, RTGS, Cheque, Cash, Fleet Card, Debit/Credit Card), bank name, cheque or transaction number, date, optional TDS, remarks. A voucher is created **pending**; the selected invoices show as **Processing**; dealer users get a "New Payment" notification. |
| 6   | Dealer   | Payments → voucher → Approve             | Checks the bank statement or cash, then approves. The voucher becomes **approved**, stamped with the approving dealer user; the invoices become **Paid** (or **Part-paid** if the amount falls short); the month's ledger is rebuilt; customer users get a "Payment Approved" notification.                           |
