...
- [x] Step 1: Content gathered / business understood — all 10 bank questions answered
- [x] Step 2: Points grouped topic-wise
- [x] Step 3: Document structure — outline approved by user
- [x] Step 4: Final document drafted → **`IPG_Proposal_UBI_VSYST.md`** (real data filled by user)
- [x] Word version generated → **`IPG_Proposal_UBI_VSYST.docx`** (designed header with company name + page numbers, Georgia/Arial typography). Outstanding before submission: fill ⟨Branch, City⟩ in the To-line; verify the two blocks the user marked "(to check)" in the .md (Uni e-Collect rails bullet in §5; onboarding/regulatory paragraph in §6).
...
## Agreed Document Outline (proposed — Step 3)

**Title:** Proposal for Internet Payment Gateway Enablement — VSYST Technologies Pvt. Ltd. in partnership with Union Bank of India (via Easebuzz)

**Target length: ~3.5 pages.** Section → content mapping → page budget:

### 1. Executive Summary (~⅓ page)
- One tight paragraph: who VSYST is, what DZZLO OMS does, the exact request (IPG enablement via UBI–Easebuzz; VSYST as master merchant, dealers as sub-merchants; settlement at UBI), headline opportunity for UBI.
- "At a glance" box: request / model / traction (130+ users, 2 dealers, 130+ daily txns) / per-dealer volume potential.

### 2. Company Profile (~⅓ page)
- KYC-style table: legal name, CIN *(dummy)*, incorporation date *(dummy)*, registered office *(dummy)*, GST *(dummy)*, PAN *(dummy)*, DPIIT/Startup India recognition, directors, contact.
- Two lines of founding story: built on 30+ years of dealership experience; proven on family-run pump first.

### 3. The Business & the Problem We Solve (~½ page)
- Today's reality at pumps: chat-app ordering, manual ledgers, lump-sum payments → long reconciliations, fraud, pending bills → bad debt.
- DZZLO OMS answer: shared bank-like ledger, immutable escalated data, bill-to-bill payments, paid/unpaid invoice filtering, per-customer amount+period credit limits, verified company/manager/driver/vehicle on every order.
- Traction line + apps live since Oct 2021.
- **Candid pre-revenue statement (user-confirmed)**: DZZLO is free by design to drive dealer adoption; monetization (subscription/API) planned post-traction. The financial story for the bank is transaction throughput, not VSYST's P&L.
- IPG = module #9 of the e-ERP roadmap; the payment leg that automates the whole loop.

### 4. The Opportunity for Union Bank of India (~½ page)
- Market: ~1,00,000 pumps; ~3,600 high-sale highway pumps carry majority of diesel sales; 50–60% already on OMC loyalty rails → target = remaining 40–50% + flows loyalty can't handle.
- Volume table: ₹10–20 lakh/month per dealer (potential ₹1 cr, ex-loyalty); UPI avg ₹5k (1k–50k); bank payments avg ₹1L (5k–5L).
- Early-bird framing: first bank to digitally enroll this merchant segment; settlement accounts = CASA/float; every dealer's customer transacts on UBI rails.

### 5. Payment Architecture & Fund Flow (~½ page)
- Marketplace model with simple flow diagram: customer → Easebuzz (RBI-authorised PA, UBI partner) escrow → direct split-settlement to each dealer's own account. **VSYST never receives, holds, or routes funds.**
- Two rails: Easebuzz gateway (UPI/smaller tickets) + Uni e-Collect virtual accounts (large bank transfers), both reconciling into DZZLO automatically.
- Refunds/chargebacks: dealer-initiated, processed in DZZLO via Easebuzz; unbroken invoice→payment→refund trail.

### 6. Risk Management & Compliance (~½ page)
- Trust story: dealers won't expose bank details; customers fear mistyped high-value transfers — gateway removes both.
- Credit discipline recap (limits, bill-to-bill, immutability) as transaction-risk reduction.
- Onboarding discipline: per-dealer sub-merchant KYC (PAN, GST, bank proof, dealership licence, agreement); platform T&C, Privacy Policy (DPDP 2023), Refund Policy; PCI-DSS & fund custody under Easebuzz's RBI authorisation.

### 7. Requests to Union Bank of India (~⅓ page, numbered list)
1. Enable IPG: VSYST as master merchant, petrol pump dealers as sub-merchants via Easebuzz; settlement at UBI.
2. Written confirmation of the UBI–Easebuzz partnership + permission for co-branded dealer onboarding material; optional joint onboarding sessions with branch officials.
3. Uni e-Collect virtual accounts for high-value transfers, with MIS/API reconciliation into DZZLO.
4. Clarify account linkage: existing current accounts vs. new dedicated UBI accounts (we are open to dealers opening UBI accounts).

### 8. Rollout Plan & Closing (~⅓ page)
- Phase 1: pilot with the 2 live dealers (130+ users, 130+ daily transactions) — low-risk validation for the bank.
- Phase 2: Raipur & Chhattisgarh dealer network.
- Phase 3: high-sale highway pumps nationally.
- Closing: request for a meeting/pilot approval; authorised signatory block + contact details.

**Design choices (rationale):**
- The ask appears twice deliberately — once in the executive summary (officers skim page 1) and once as the formal numbered section 7.
- Every bank objection is answered before section 7: legitimacy (§2), business reality (§3), why bother (§4), where the money goes (§5), what can go wrong (§6).
- Tables/diagram used in §2, §4, §5 to keep page count down.
