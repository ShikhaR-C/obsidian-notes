...
- **Colours are tokens, never literals** (2026-09-06): a screen or component takes a palette token NAME and a test asserts `themes.light.colors.<token>` / `tint(token, alpha)`. A hex pasted from a design mock passes today and lies the day `src/theme/tokens/palette.js` moves — when a mutation smoke shows a test missed a colour swap, add the token assertion, never the literal.
- **Text sets the box — test at a stated font scale** (2026-09-06, F-APP-7): RN's Jest preset has always reported `fontScale: 2`, so every screen test runs at 2× whether it says so or not. `renderScreen(ui, { fontScale })` makes the scale an input; a decision test for anything that holds text runs at `1` (the signed-off design) and `2.143` (iOS Accessibility Large) and asserts the container has no fixed `height` — a `minHeight`/padding derived through `useTypeScale`, digits `numberOfLines={1}`, pairs stacked at `ACCESSIBILITY_FONT_SCALE` (1.786). The layout equation of a screen (`layout.js`) is pinned at `[1, 1.353, 1.786, 2.143, 3.571]`.
