...
| `date-range-sheet` | no live file — `date-range-sheet/01-preset.png` (the "Filter by Date" bottom sheet with a preset chosen: header + Close, the summary of the chosen window, the five choices Custom · Today · Yesterday · Last 7 days · Last 30 days as radios, the "Preset Time" card with 6 - 6 AM / 12 - 12 AM, the footer Clear All + Apply; from `~/Downloads/Payments 1a Copy-selection-2.png`) and `date-range-sheet/02-custom.png` (Custom chosen: "4 days selected", From and To rows of a date field and a time field, the other choices pushed under the footer; from `~/Downloads/Payments 1a Copy-selection.png`) — both dropped by the user 2026-09-20. **Not a screen: a common v2 component**, first used by Daily Summary. Reference only (colours from the theme, states and rules from the plan); read into [[../09-date-range-sheet-plan]] | 2026-09-20  |
