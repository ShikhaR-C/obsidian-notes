...
| **Screen (Tier 3)**        | the screen renders each _decision_ correctly against a real store + navigation + MSW                    | —                                                                                | `src/screens/v2/<Name>/__tests__/<Name>.test.js`     | RNTL + MSW + `renderScreen()`                                  | ~1 s         | red before the component file exists                                         |
...
6. **Cutover test.** The navigator resolves the v2 component for the route name, and the v1 component when the screen's `screen_v2_<slug>` toggle is `false` (house toggles from the superadmin website, default on — D10 in [[00-overview#Decisions]]).
