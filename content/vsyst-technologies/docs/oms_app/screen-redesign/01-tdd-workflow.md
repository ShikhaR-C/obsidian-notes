...
**Outcome:** one way of working, written down once, that every screen iteration follows in both repos. It extends the house rules in [[../tdd-testing-guide|app TDD guide]] and [[../../oms_api/tdd-testing-guide|API TDD guide]] to a project where the _contract is being designed_, not just protected.
...
2. **Push every rule to the lowest layer that can prove it.** A business rule (credit, diesel limit, closed period) lives in an API integration test. A screen-side decision (what to show for `creditState`) lives in an app util test. The screen test only proves the screen _reacts_ to the util's answer. Never prove the same rule at two layers.
3. **Fixtures are the contract between the repos.** Every v4 response the app consumes is captured from the seeded world (`yarn fixtures:export` → `fixtures/api_v4/*.json`) and pulled into the app (`yarn fixtures:pull`). Hand-rolled success bodies are forbidden in app tests; hand-rolled _error_ bodies are allowed inline.
...
7. **v3 is frozen for features.** Redesign work never edits `api_v3/routes` or `api_v3/controllers`; it _calls_ `api_v3/services`. Any change a new screen needs inside a v3 service is a separate, test-first PR with its own flow-map row, because it changes behaviour for v1.78 clients too.
...
| **Screen (Tier 3)**        | the screen renders each _decision_ correctly against a real store + navigation + MSW                    | —                                                                                | `src/screens/v2/<Role>/<Name>/__tests__/<Name>.test.js`     | RNTL + MSW + `renderScreen()`                                  | ~1 s         | red before the component file exists                                         |
...
Same loop, plus: the precondition has a red case _and_ a green case, the Mongo side effect is asserted directly (not just the status), the idempotency replay is a no-op test (bug-museum #1 lesson), and the invalidation contract (which read models change) is asserted through a follow-up read in the same test.
...
2. **Tier 1 first.** List every "Derived?" cell in spec §2 and every precondition in §3 that is _evaluated on the client_. Each becomes a pure function under `src/utils` or `src/helpers` with a red test, then green. Watch mode stays on.
...
4. **Tier 3, red first.** `renderScreen(<Name />, { preloadedState, handlers })` from `src/test/testUtils.js`. Write one test per _decision row_ (conditional element, precondition true/false, loading/empty/error, navigation on success). The component file does not exist yet, so the suite is red.
...
6. **Cutover test.** The navigator resolves the v2 component for the route name, and the v1 component when the screen's `screen_v2_<role>_<slug>` toggle is `false` (house toggles from the superadmin website, default on — D10 in [[00-overview#Decisions]]).
7. **Mutation smoke** (flip a decision helper, watch Tier 1 _and_ Tier 3 go red), PR with the template checklist.
...
- The API PR for screen N+1 may start while the app PR for screen N is open. Two _app_ PRs for different screens should not be open at once until the team has shipped three screens this way.
...
| **New:** fixtures freshness    | `fixtures.meta.json` ≤ 14 days old at release time, for `api_v3` _and_ `api_v4` sets |
