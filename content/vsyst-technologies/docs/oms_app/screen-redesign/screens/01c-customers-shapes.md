...
| **any width / `short`** — phone on its side, Flip cover, dragged iPad window | **the yield ladder** (07 S3): chrome yields until `MIN_ROWS` fit; search and list never yield         | to build — _"according to plan"_        |
...
3. **The filter chips, each a full-width row** rather than a wrapping line: one row per filter, then the sort row under them. The sort row is decision 73's full-width sort control (name at the left, direction arrow at the right).
...
| P1   | `Customers/index.js`, `components/CustomerRow.js`, `FilterChips.js`, `SummaryStrip.js`, `Customers/layout.js` | the `expanded / tall` panel of §2: `PANEL_WIDTH` in points; the panel header row (`headerShown: false` in this cell); chips as full-width rows; summary forced stacked; the row's inline arrangement.                          |
