...
- **v4 contract set (screen-redesign Phase 1, PR #39, 2026-09-04):** `test/api_v4/temp/fixtures.captures.js` → `yarn fixtures:export:v4` → `fixtures/api_v4/{ping,app_features,fixtures.meta}.json`; drift detector `test/api_v4/contract/fixtures.test.js` on the shared scrub `test/helper/contract.js` (v3 volatile keys + `generatedAt`); `scripts/check_fixtures_fresh.js` checks both sets (v4 is app-only, pulled as `v4_*.json`). Full v4 test idiom in `docs/testing.md` §11.
