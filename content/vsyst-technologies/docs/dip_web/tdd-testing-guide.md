...
> **Status (verified 2026-09-01):** the entire testing stack lives on branch **`web_tdd`** (3 commits, working tree clean, in sync with origin). **PR #23 is OPEN and unmerged** since 2026-07-10, and the branch is now **6 commits behind `main`** — PRs #24/#25/#26 (the whole Ledger Check feature, incl. the `istDayEnd` credit-window fix `b513fba`) landed on `main` afterwards. **`main` has zero tests today.** Suite on `web_tdd`: **5 files / 32 tests, ~1.5 s** (budget ≤ 1 min). Built under tasks_12 (see `../tasks/tasks_12_tdd_testing/`), Phase 3 + 5.
...
| **Bugfix**                           | **Mandatory test-first** — component-level if visual/behavioral, hook/util-level if logic. The test is never deleted. ⚠️ Standing counter-example to repair: the `istDayEnd`/`ledgerCheck.js` fix (`b513fba`, merged via PR #26) shipped with **no test** — `src/utils/ledgerCheck.js` doesn't even exist on `web_tdd`. First task after rebasing onto `main`: write the `istDayEnd` + credit-window unit tests. |
...
1. **Merge PR #23** — open ~7 weeks and 6 commits behind `main`; rebase/merge `main` into `web_tdd` first (picks up Ledger Check, `ledgerCheck.js`, pull-to-refresh etc.), re-run the suite, then merge. Until then `main` has zero tests, no test CI, no runbook.
2. **`istDayEnd` untested** (see playbook) — the top new-test priority after the rebase.
...
5. **Generated fixtures stale** (2026-07-09, > 14-day threshold) and currently unused; `src/test/fixtures/factories.js` is dead code until the transaction tests land.
