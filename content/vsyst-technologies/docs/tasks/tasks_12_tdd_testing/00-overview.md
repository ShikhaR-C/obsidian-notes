...
> **Re-verified against repos 2026-07-09.** Two drift corrections: (a) the tasks_08 `max_cr_lmt` redesign is **no longer pending — it landed in all three repos** (API `bf063d4` 2026-06-20 + seed fix `1169ceb`; app v1.78 `src/helpers/Credit`; dip-web PR #18/v1.4.5) — flow #8 and Phase 2 §2.2.3/§2.3.1 and Phase 4 §4.2 amended to pin the _landed_ contract; (b) dip-web has an **unmerged Vitest spike** (`origin/claude/transaction-tests-issue-10`, +2,261 lines) that Phase 3 must triage before building (new §3.0). Working branches `api_tdd`/`web_tdd` (API repo) and `app_tdd` (app repo) exist — created at mainline heads, no commits yet.
>
> **IMPLEMENTED 2026-07-09 → 2026-07-10 (all 7 phases; uncommitted, in the working trees of `api_tdd` / dip-web `web_tdd` / `app_tdd`).** Suite state: **API 668** (`yarn test:full`, fresh seed) · **web 32** (Vitest) · **app 337** (Jest). The cross-repo `release_gate.sh` runs **green (PASS/exit 0)** and **red (BLOCKED/exit 1)** as designed. Each repo's _only_ production-source change is the one authorized `makeStore()` factory (`src/store/apis/index.js`) — everything else is tests, fixtures, scripts, CI workflows, and docs. Per-phase implementation notes live at the bottom of each `NN-phase-*.md`. Notable finds during build: v3 `protect`/`authorize`/`scope` are unwired (flow #2 pinned as _current behavior_ per user call); the DB-driven version gate is absent from these branches (only the hardcoded `check_user_version` exists); the jsdom↔undici `AbortSignal` realm clash needed a custom Vitest env (`dip-web/src/test/env/jsdom-fetch.js`); `yarn uproot` was silently a no-op on Node ≥ 22 (`fs.rmdirSync` recursive removed) — fixed; and the Phase-5 contract spec had to scrub non-deterministic ObjectIds/`inv_no` to survive `test:full`'s re-seed (Phase 5 §Correction). **Deferred by design:** e2e smoke (Q6), DIP test-mounting (Phase 2b), several front-end carry-over tests, team ratification (Q10), and release-cut/CI-orchestration mechanics (Q8). **Status flips to "operational" after two real releases run the gate + checklist.** Nothing is committed — the workspace awaits the user's review/commit.
> **COMMITTED & PR'd 2026-07-10** (API `api_tdd` → PR #35, dip-web `web_tdd` → PR #23, app `app_tdd` → PR #48; CI green on all three, no repo secrets needed). **MERGED 2026-09-03** as the screen-redesign Phase 0 prerequisite (`../../oms_app/screen-redesign/00-overview.md` §Phase 0) — into the integration branches the user retargeted the PRs to: API `slave` (`2ca0301`), app `slave` (`a7c32d7`), dip-web `slave_dev` (`23ac5b0`); `master`/`main` receive them at the next release merge. Before merging: fixtures re-exported on seed `v3_2026-09-03` and pulled into both front-ends, and the `istDayEnd` regression test dip-web owed since PR #26 was added (`2b5efec`). Gate state at merge: **API 778 passed / 47 skipped / 1 todo · web 46 · app 337 · `release_gate.sh` PASS**. Per-repo status lives in `docs/{oms_api,oms_app,dip_web}/tdd-testing-guide.md`.
...
| **New API endpoint / service rule**   | **Test-first at integration level.** Write the supertest spec from the contract (status, envelope, side effects on Mongo), then implement. The endpoint contract _is_ the design artifact.                      |
...
| **Feature**                             | A multi-step **business flow** spanning several resources works end-to-end _within one system_.            | `test/api_v3/features/` convention: e.g. order → SO → invoice → voucher → ledger balances                                                             | A user journey across screens (login → place order) with MSW                                                                                | A journey (sign-in → create decant) with MSW                                    | 1–10s     |
| **E2E**                                 | Real processes wired together: UI ↔ real API ↔ real (local) DB.                                            | n/a (API _is_ the backend)                                                                                                                            | Maestro/Detox on emulator against a **locally seeded API** — no e2e yet (✅ Q6)                                                             | Playwright against Vite build + locally seeded API — no e2e yet (✅ Q6)         | minutes   |

**How to plan across the four layers:** push every rule as far _down_ the table as it can live. A credit-limit formula belongs in a unit test; "POST /order_msts rejects when over limit" belongs in an API integration test; "customer sees the blocked-credit message" belongs in one RNTL screen test; only "the whole stack boots and a login works" belongs in e2e. Never re-prove the same rule at two layers — the upper layer assumes the lower one.

### Shape: trophy, not pyramid

Because the API is the system of record and both clients are thin views over RTK Query, the biggest investment is the **API integration/feature layer** (fast, local, already idiomatic here), flanked by cheap unit tests everywhere, a modest set of RTL/RNTL screen tests, and a _tiny_ optional e2e smoke. Roughly:
...
| 8   | P0   | **Credit engine**: `max_cr_lmt` semantics (tasks_08 null=unlimited/0=blocked/>0=capped — **landed** in API `bf063d4` 2026-06-20), `cr_bill_lmt`, `max_cr_days`, `adv_dep` balance, `legacy_credit_presenter` (≤1.77 shim) | `dealer_custs`, services, `helpers/middlewares.js`    | 🟡 — blocking-path & presenter tests missing                         |
...
- Gaps: the test app **skips the production middleware chain** (`api_key_v1`, `logging()` → so `protect` has no `req.loggedInUser`, `check_user_version`), meaning role/scope/bearer and version-gate behavior are untested (flows #2, #13). Nuance (re-verified 2026-07-09): `legacy_credit_presenter` is wired **inside** `api_v/api2.js`/`api_v/api3.js` and self-parses the `meta` header, so it _is_ already reachable through the test app — only the three globals above need §1.2.
- Landed since the prompt was written (re-verified 2026-07-09, undocumented in tasks_08 until now): **tasks_08 credit semantics are live** — v3 order create/update enforce null=unlimited / 0=blocked / >0=capped (`api_v3/services/order_msts.js` ~786/922), `createDC` defaults new relations per client version (`legacyCredit ? null : 0`, `services/dealer_custs.js` ~1442), presenter shim in both routers (`bf063d4`); seed explicitly sets `max_cr_lmt: null` so seeded orders keep flowing (`1169ceb`, `test/api_v3/temp/seed/v3/index.js`). A `scripts/` dir already exists (`migrate_max_cr_lmt.js`, `backfill_advdep_seed_opening.js` + `backfill:advdep` script) — `release_gate.sh` (Phase 6) joins it, it does not create it.

**`dzzlo_oms_app`** — RN 0.84 / React 19 / RTK Query (18 `injectEndpoints` files) / AsyncStorage tokens. Jest works but only Firebase is mocked in `jest.setup.js`; `@testing-library/react-native` not installed; `__tests__/App.test.tsx` is a render-only smoke. Rich, immediately-testable pure layer under `src/utils`, `src/helpers`, `src/store` — including the shipped tasks_08 tri-state credit helper (`src/helpers/Credit/index.js`: `creditState`/`isUnlimited`/`isBlocked`/`isCapped`/`creditUtilization`, adv_dep counted as spending power per `6a8109a8`, in v1.78; CustSettings already has the Unlimited/Fixed-Limit control). ⚠️ `src/components/ImagePicker` imports `react-native-image-picker` + `react-native-permissions` which are **not in package.json**. ⚠️ `yarn test` runs `APP_ENV=testing` whose `.env.testing` points at the **remote staging URL** — harmless only if tests never hit the network (Phase 4 enforces this with MSW `onUnhandledRequest: 'error'`).

**`dip-web`** — the "see Phase 5" mystery is solved: it refers to `src/docs/plans/cra-to-vite-migration.plan.md` → **"Phase 5: Testing (Deferred)"**, which already prescribes **Vitest + jsdom + `src/setupTests.js`** and keeps the installed RTL deps. Phase 3 of this plan executes exactly that (plus MSW). Single `createApi` (`dip-api`), 21 `injectEndpoints` files (~120 endpoints, 89 mutations), tokens in localStorage, roles dealer/superadmin with `dip.<resource>.<action>` permissions, JSX-in-`.js` (esbuild loader — Vitest inherits it via `vite.config.js`). No Vite proxy; all env API URLs are remote → MSW must intercept everything. **Prior art (found 2026-07-09):** unmerged remote branch `origin/claude/transaction-tests-issue-10` already contains a Vitest spike (+2,261 lines: `vite.config.js` test block, `src/setupTests.js`, `src/test-utils/{renderWithProviders,mockApi}`, store tests for decants/insps/meter_reads, page tests for InspsList/MeterReadList) — Phase 3 §3.0 triages it before building. Also landed: tasks_08 tri-state credit UI (`fix/maxcrlmt` merged as #18, shared `computeCreditProgress` util, v1.4.5).

**Vault** — `tasks_11_partner_api` format copied here: no frontmatter, `00-overview.md` + `NN-phase-N-slug.md`, `**Outcome:**` line, `## N.M` subsections, `## Phase N checklist`. Deviation (required by the source prompt): each phase file adds an `**Effort:**` estimate line. Related plans to honor: tasks_07 (AdvDep — seed factory suggestions; Phases 1–2 implemented 2026-05-25, Phase 3 = v2 spec'd), tasks_08 (max_cr_lmt semantics — **landed in all three repos**, so credit tests/seed pin the new contract directly; tasks_08 status line synced 2026-07-09), tasks_02 `05-cicd-github-actions.md` (existing CI plan to reconcile in Phase 6), tasks_10/tasks_11 per-phase testing sections.
